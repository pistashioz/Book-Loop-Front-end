...
<script setup>
import { ref } from 'vue';
import { createPublisher } from '~/composables/api/adminService'; 
...
try {
    const response = await createPublisher(publisherName.value)
    if (response) {
