<template>
            ...
            <form @submit.prevent="updateName" class="p-4 md:p-5">
              <div class="grid gap-3 mb-4 grid-cols-1">
                ...
                <div class="col-span-3 sm:col-span-1">
                  <label for="genreName" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                  <input v-model="genreName" type="text" name="genreName" id="genreName" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Update Person's Name" required>
                </div>
  ...
  <script setup>
  import { ref } from 'vue';
  import { updateGenre } from '~/composables/api/adminService'; 
...
const updateName = async () => {
  try {
    const response = await updateGenre(props.genreId, genreName.value);
