<template>
  <div class="group relative" v-if="work">
      ...
      <div class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none lg:h-80">
        <NuxtLink :to="`/works/${work.workId}`" id = 'nuxtlink'>
        <img
          v-if="coverImage"
          alt="Book Cover"
          :src="coverImage"
        ...
        <span class="buttonOptions inline-flex overflow-hidden rounded-md border bg-white shadow-sm">
  <button
  ...
  <button
    ...
    title="Delete Product" @click ="deleteWork(work.workId)"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
